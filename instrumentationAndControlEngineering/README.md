...
という流れになることが多い。  
しかし、制御設計の流れでは、  
ア: <!-- hole 制御対象の特性を解析 --> →仕様→設計→製作・実装→評価  
となる。このことから分かる通り、イ: <!-- hole 制御対象 --> ありきである。  

一般に制御系では、入力u(t)が与えられると、それに対応した出力y(t)を生じる。  

これを方程式の形に表すのをウ: <!-- hole 制御系のモデル化 --> という。  

### 系の数学モデル化

・バネ―マス―ダンパ系  
バネ―マス―ダンパ系は、次のような系である。  
[ア](img/bmd.png)  

ばねによる力をfK(t)、ダンパによる減衰力をfD(t)とすると、  
...
[ア](img/1000k.png) の形で表される。多くのシステムでこれが言える。  

α、βを実数とする。  
イ:<!-- hole 重ね合わせの原理 --> つまり ウ:<!-- hole αu1(t)+βu2(t)を加えたとき、出力が αy1(t)+βy2(t)となる --> が成り立つシステムを線形システムとか線形エ: <!-- hole ダイナミカルシステム --> という。  

線形システムでは、ウより [オ](img/1000kk.png) のような式が成り立ち、この式に当てはめて考えることをカ: <!-- hole 線経常微分方程式でモデル化 --> するという。  


(非線形系は、特定の区間を線形とみなして利用。)  


###ラプラス変換 

ここで、ラプラス変換とは、ア: <!-- hole 微分をs倍、積分を1/s倍に置き換える --> ことで微分方程式を簡単な方程式に変換してとく演算子法のことである。

ラプラス変換の公式は、 [イ](img/laplace.png) の通り。  

伝達関数は ウ:<!-- hole 出力のラプラス変換÷入力のラプラス変換 --> といえる。  
