...
名詞、動詞:(説明を省く)  
形容詞:名詞を説明する飾り。ア: <!-- hole なくても英文自体は壊れない。 -->  
副詞:名詞以外を説明する飾り。ア  
他:その都度説明する。  
...
C:イ <!-- hole 形容詞か名詞 -->  
O:ウ <!-- hole 名詞 -->  
M:形容詞か副詞(か、エ <!-- hole 前置詞＋名詞 --> )  

### 文型

ア: <!-- hole SV、SVC、SVO、SVOO、SVOC --> のいずれか。  
言ってしまえば、  
英語の正しい文=SV(C|(O(O|C)?))?  

構文解析するとき、Vは絶対に動詞。  
→イ: <!-- hole 動詞 --> を知っておけば、動詞より左はS、動詞以降はOやらCやら、と(だいたい)わかる。  
さらに、SとOは絶対名詞だし、Cは形容詞か名詞であるが、名詞は種類が多すぎる。

### 節

ア: <!-- hole S+Vの塊 --> 。文⊃節。  
イ: <!-- hole 1つの文の中に複数の節 --> があったりする。  
それを接続するのがウ: <!-- hole 従位接続詞 -->  
「主節」「ウ」「従属節」  
あるいは  
「ウ」「従属節」,「主節」の順番  
ちなみに、エ: <!-- hole Sの欠けた節 --> のことを「句」という。厳密にはそうじゃないけど気にしない。  
...
不定詞の役割  
動詞のくせに、ア: <!-- hole あたかも別の品詞であるかのように振舞う -->  
今回は副詞として。  
...
andはア: <!-- hole 等位接続詞 -->  
「A and B」はイ: <!-- hole 文法上対等 --> なAとBを結びつける。  
...
Sが欠けていて、正しい文型にない→ウ: <!-- hole andの前から借りてくる(S=this data) -->  

andの後の訳:このデータはたくさんの準備を必要としない  

andを使わずに書いてみると次のとおり。  

エ: <!-- hole This_data_is_somewhat_simpler._This_data_does_not_need_a_lot_of_preparation. -->  

全体の訳:このデータは、なんか比較的単純で、面倒な準備が不要だ  

今回のA and Bは、  
A=オ: <!-- hole is_somewhat_simpler -->  
B=カ: <!-- hole does_not_need_a_lot_of_preparation -->  
だったわけだ。  
...
whichはア: <!-- hole 関係代名詞または疑問代名詞、または疑問形容詞 -->  
...
実は、「疑問*詞+名詞」と来ている場合、この疑問*詞は疑問ケ: <!-- hole 形容 --> 詞つまりケ詞となる。  
whichという形容詞が、確かにcolumnという名詞を修飾している。  
そして、疑問形容詞+名詞(に限らず、疑問詞全般)はコ: <!-- hole 本来後ろに来るはずのものが前に出てきてしまっている --> ということができる。  
従って、元に戻すと  
...
蛇足:一つの文が長いししかも  ア:<!-- hole 文中に等位接続詞が複数回出てくる --> のは、文法的に不親切。  
ライティングの参考にはすべきでない  
...
イディオム:イ <!-- hole hover_the_mouse_over_A --> →「Aの上にカーソルをのせる」  
...
したがって、このwhichはア: <!-- hole 関係代名詞 --> である。  

アは従位接続詞と代名詞両方の役割を果たす。  

従位接続詞は従属節の先頭に来て、イ: <!-- hole 従属節全体を副詞節や名詞節などにしてしまう。-->  
...
今回の文の場合は、which以下はウ: <!-- hole 副詞節となる(which以下を丸ごと消しても文は壊れない) --> 。  
...
まずwhileはア: <!-- hole 接続詞または名詞 --> である。  
動詞「～をダラダラ過ごす」はめったに使われない。  
従位接続詞として使われる場合は イ: <!-- 副詞節　-->を作る(＝要はwhile以下が副詞的になる)  
...
In its Parameters:  
は、ア: <!-- hole 2～4で、説明が続いている --> ことを意味する。  
